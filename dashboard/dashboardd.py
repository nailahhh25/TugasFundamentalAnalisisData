...
@st.cache_data
def load_data():
    main_df = pd.read_csv("data/main_data.csv", parse_dates=["order_purchase_timestamp", "order_month"])
    return main_df

@st.cache_data
def load_rfm():
    orders_df    = pd.read_csv("data/orders_dataset.csv",   parse_dates=["order_purchase_timestamp"])
...
with col_right:
    ...
    st.dataframe(revenue_by_cat, use_container_width=True, hide_index=True)
